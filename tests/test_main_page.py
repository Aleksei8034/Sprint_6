...
class TestMainPage:
    @allure.title('Клик на кнопку "Самокат", переход на главную страницу')
    @allure.description(
        '"Заказать" кнопку в хедере страницы, заполняем страницу "Для кого самокат", клик на иконку "Самокат" в хедере, переход на главную страницу')
    ...
    @allure.title('Переход на страницу дзена по клику на "Яндек" в хедере')
    ...
    def test_click_on_yandex_icon(self, driver):
        ...
        main_page.wait_for_new_window(Urls.dzen)
        assert main_page.get_url() == Urls.dzen
