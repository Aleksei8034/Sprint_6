...
class TestOrderPage:
    ...
    @allure.title('заказ самоката, кнопка вверху страницы')
    @allure.description(
        'Запустили браузер, клик на "Заказать" в верху страницы, заполнили форму клиента, клик "Далее", проверяем '
    ...
    @allure.title('закз самоката, кнопка по середине страницы')
    @allure.description(
        'Запустили браузер, клик на "Заказать" по середине страницы, заполнили форму клиента, клик "Далее", проверяем '
        'переход на следующую страницу "Про аренду"')
    @pytest.mark.parametrize("user_data", [positive_user_data_1, positive_user_data_2])
    def test_positive_make_order_click_on_center_button(self, driver, user_data):
        main_page = MainPage(driver)
        order_page = OrderPage(driver)
        main_page.click_on_cookie_button()
